...
// 下段: シフト提出 / 経営メニューへ切替 / 意見箱
//   ※ v5 で「自分のランク」だった中央下を「経営メニューへ切替」に変更
//     ランク確認はダッシュボードから到達可能で重複だったため。
//     経営者切替テキストコマンドへのショートカット動線を復活させる。
// ─────────────────────────────────────────
export const STAFF_MENU_DEF = {
  ...
  areas: [
    ...
    { bounds: { x: C[1], y: R[1], width: W[1], height: H }, action: { type: 'message', label: '経営メニューへ',  text: '経営メニューへ切替'               } },
